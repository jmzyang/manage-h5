<template lang="pug">
  .page-wrap.meeting-create(ref="layoutPage")
    BarHeader(
      ...
      :backEvent="true",
      @back="handlerBack",
      :title="isEdit ? $t('revisionMeeting') : $t('appointmentMeeting')")
      //- :: 修改会议 ，:: 预约会议
      template(slot="actions")
        .header-cancel(@click="closeVisible = true", v-if="!isEdit") {{$t('cancel')}}
        //- :: 取消
    .meeting-create-form
      .input-block
        .input-item(@click="selectItem('theme')")
          .input-item-con
            textarea(
              :placeholder="$t('meetingThemeInput')" 
              maxlength="128",
              ...
              //- :: 请输入会议主题(128字内)，:: 会议主题
            span.tips {{ errors.first('theme') || ''}}
        .input-item(@click.stop="chooseFormItem('classification')")
          .input-item-con
            span.item-title.classification {{$t('meetingClassification')}}
            //- :: 会议分类
            .input-con(
              name="classification"
              v-model="classVal")
              | {{ classVal ? `${classVal}` : $t('pleaseChoose') }}
              //- :: 请选择
              i.iconfont.icon-arrow-right.arrow-right
            span.tips {{ errors.first('classification') || ''}}
        .input-item(@click="selectItem('type')")
          .input-item-con
            span.item-title.video-type {{$t('meetingMode')}}
            //- :: 会议模式
            .radio-wrap
              van-radio-group.radio-group(v-model="formData.videoConference" :disabled="isEdit")
                van-radio(name="2") {{$t('localMeeting')}}
                //- :: 本地会议
                van-radio(name="1") {{$t('videoMeeting')}}
                //- :: 视频会议
      .input-block
        .input-item(@click="selectItem('startTime')")
          .input-item-con
            span.item-title.starttime {{$t('startTime')}}
            //- :: 开始时间
            .input-con {{ formData.startTime ? formData.startTime.substring(0,16) :  $t('timeChoice') }}
              //- :: 请选择时间
              i.iconfont.icon-arrow-right.arrow-right
        .input-item(@click.stop="chooseFormItem('duration')")
          .input-item-con
            span.item-title.duration {{$t('meetingDuration')}}
            //- :: 会议时长
            .input-con(
              name="duration"
              ...
              | {{ formData.duration ? `${formData.duration}${$t('minutes')}` : $t('durationChoice') }}
              //- :: 分钟 :: 请选择
              i.iconfont.icon-arrow-right.arrow-right
      ...
      .input-block
        ...
        .input-item(@click="chooseFormItem('conferenceHall')")
          .input-item-con
            span.item-title.room {{$t('conferenceHall')}}
            //- :: 会场
            .input-con(
              name="room"
              ...
              data-vv-name="room")
              | {{meetingRoom.length ? `${$t('selected')} ${meetingRoom.length} ${$t('PCS')}${$t('conferenceHall')}` : $t('pleaseChoose')}}
              //- :: 已选择，::个会场，::请选择
              i.iconfont.icon-arrow-right.arrow-right
            span.tips {{ errors.first('conferenceHall') || roomValidate}}
        .input-item(@click.stop="chooseFormItem('serviceItems')")
          .input-item-con
            span.item-title.service-items {{$t('services')}}
            //- :: 服务项
            .input-con(
              name="serviceItems")
              | {{ serviceItems.length ? `${$t('selected')} ${serviceItems.length} ${$t('PCS')}${$t('services')}` : $t('pleaseChoose') }}
              //- :: 已选择，::个服务项，::请选择
              i.iconfont.icon-arrow-right.arrow-right
      .input-block
        .input-item(@click="chooseFormItem('convenor')")
          .input-item-con
            span.item-title.host {{$t('host')}}
            //- :: 主持人
            .input-con(
              name="convenor",
              ...
              v-validate="'required'",
              :data-vv-as="$t('host')"
              data-vv-name="convenor")
              | {{convenor.total ? `${$t('selected')} ${convenor.total} ${$t('PCS')}${$t('host')}` : $t('pleaseChoose')}}
              //- :: 已选择，::个主持人，::请选择
              i.iconfont.icon-arrow-right.arrow-right
            span.tips {{ convenor.total ? '' : errors.first('convenor')||''}}
        .input-item(@click="chooseFormItem('leader')" v-if="KIparameter.MAIN_PARTICIPANTS == 1")
          .input-item-con
            span.item-title.leader {{$t('attendeeLeader')}}
            //- :: 参会人领导
            .input-con(
              name="leader"
              v-model="formData.leader"
              :data-vv-as="$t('attendeeLeader')" 
              data-vv-name="leader") 
              | {{ leader.total ? `${$t('selected')} ${leader.total} ${$t('PCS')}${$t('attendeeLeader')}` : $t('pleaseChoose')}}
              //- :: 已选择，::位参会人，::请选择
              i.iconfont.icon-arrow-right.arrow-right
        .input-item(@click="chooseFormItem('participants')")
          .input-item-con
            ...
            .input-con(
              ...
              | {{participants.total ? `${$t('selected')} ${participants.total} ${$t('PCS')}${$t('attendee')}` : $t('pleaseChoose')}}
              //- :: 已选择，::位参会人，::请选择
              i.iconfont.icon-arrow-right.arrow-right
            span.tips {{ participants.total ? participantsValidate: errors.first('participants') || ''}}
      .input-block
        .input-item(@click="chooseFormItem('issue')")
          .input-item-con
            span.item-title.issues {{$t('issue')}}
            //- :: 议题
            .input-con(
              name="issue",
              ...
              :data-vv-as="$t('issue')" 
              data-vv-name="issue")
              | {{meetingIssue.length < 1 ? $t('pleaseFill') : $t('fillIn')}}
              //- :: 请填写，::已填写
              i.iconfont.icon-arrow-right.arrow-right
      .input-block(v-if="+KIparameter.OPEN_MEETING_NOTICE !== 1")
        .input-item
          .input-item-con
            span.item-title.notice {{$t('noticeMethod')}}
            //- :: 通知方式
            .input-con(name="notifyMode" v-model="formData.notifyMode" v-validate="'required'" :data-vv-as="$t('noticeMethod')" data-vv-name="notifyMode")
              .note-mode(:class="{active:selectNotifyMode['2']}" @click="selectNotifyMode['2']=!selectNotifyMode['2']") {{$t('mail')}}
              //- :: 邮件
              .note-mode(:class="{active:selectNotifyMode['1']}" @click="selectNotifyMode['1']=!selectNotifyMode['1']") {{$t('shortMessage')}}
              //- :: 短信
              .note-mode(:class="{active:selectNotifyMode['0']}" @click="selectNotifyMode['0']=!selectNotifyMode['0']") {{$t('inApplication')}}
              //- :: 应用内
            span.tips {{ errors.first('notifyMode') || ''}}
    .commit-wrap
      ...
      button.submit(type="danger", @click="submit" v-loading.fullscreen.lock="loading") {{$t('release')}}
      //- :: 发布
    //- 事件选择器
    van-popup(
...
export default {
  data () {
    return {
      isEdit: this.$route.name === 'meeting.edit' ? true : false, // 是否为修改会议
  ...
  methods: {
    ...
    //       page: 1,
    //       pagecount: 10,
    //       PARAM_CODE:'OPEN_READINESS_TIME',
    //       mask: false // 禁用全局菊花
    //     }
    //   ]
    //   let result = await this.$ajaxRequest(req)
    //   if (result.code === '0') {
    //     this.openReadinessTime = result.data[0].PARAM_VAL
    //   }
    // },
    // 获取会议分类信息
    ...
    saveStorageFormData (isSave) {
      ...
      const convenor = isSave ? this.convenor : { organization: [], member: [{
