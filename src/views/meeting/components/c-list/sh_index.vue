      ...
      Scroll(
        ...
        .yearList(v-for="(YValue,YIndex) in meetingListData")
          ...
          .dayList(v-for="(DValue,DIndex) in YValue.data")
            p(v-if="calendarShow==='nav'") {{DValue.day}}
            .listMain(
              ...
              .mainHeader(:class="item.stateRgb")
                ...
                //-   会议类型 span {{item.conferenceType}}
                .headerRight(@click.stop='showCheckedStatus(item.state,item)') {{item.timeSlot.substring(0, 5)}}
              .mainContent
                .headerLeft {{'创建人:' + item.founderName}}
                .headerRight(:class="STATEKEY[item.state]") {{item.state}}
