<template lang="pug">
  Scroll.top-contacts-list(v-loading="loading" :loading-text="$t('loading')")
    .contacts-section(v-if="enterpriseData.length")
      van-checkbox.contacts-check-all(v-model="checkedAllEnterprise") {{$t('internalContacts')}}
        //- :: 内部联系人
        .iconfont(@click.stop="inShow = !inShow" :class="inShow ? 'icon-arrow-top' : 'icon-arrow-bottom'")
    ...
    .contacts-section(v-if="externalData.length && isShow")
      van-checkbox.contacts-check-all(v-model="checkedAllExternal") {{$t('externalContacts')}}
        //- :: 外部联系人
        .iconfont.icon-arrow-bottom(@click.stop="outShow = !outShow" :class="outShow ? 'icon-arrow-top' : 'icon-arrow-bottom'")
      .contacts-list(v-show="outShow")
        van-checkbox.contacts-item(
...
export default {
  ...
  computed: {
    // 是否显示常用联系人
    isShow () {
      return this.selectType === 'participants' || this.selectType === 'participantsEdit' || this.selectType === 'participantsIssue'
    },
    // 内部联系人全选
