<template lang="pug">
.page-wrap.meeting-home(v-loading="loading")
    BarHeader(
        layout="title",
        :border="true",
        :close="true",
        ...
        template(slot="actions")
            .submit-header-meeting-room(@click.stop="submit") 加入会议
    template(v-if="!getmeetingList")
      .meeting-list-container
        .meeting-bg
        .meeting-des
          .des-title 在线会议
          .des-normal 让会议不受空间限制
        .all-meeting(@click.stop="goMeetHistory") 全部会议
    ...
    template(v-else)
        ...
        .all-meeting(@click.stop="goMeetHistory", style={'padding-top': '30px'}) 全部会议
          i.iconfont.icon-arrow-right.arrow-right
    .commit-wrap
      button.btn.immediately(type="danger") 立即会议
      button.btn.reservation(type="danger", @click="createMeet") 预约会议
</template>
<script>

import BarHeader from '@/components/c-header'
import SpCalendar from '@/components/c-calendar/'
import MeetingList from './components/c-list/sh_index'
